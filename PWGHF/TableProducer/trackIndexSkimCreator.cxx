    ...
    for (const auto& collision : collisions) {
      ...
      for (const auto& bachIdx : groupedBachTrackIndices) {
        ...
        for (const auto& v0 : V0s) {
          ...
          // fill table row
          rowTrackIndexCasc(thisCollId, bach.globalIndex(), v0.globalIndex());
          // fill histograms
          if (fillHistograms) {
...
struct HfTrackIndexSkimMcMatchQC {
  ...
  void init(InitContext const&)
  {
    if (doprocessMcMatch && doprocessNull) {
      LOG(fatal) << "processNull and processMatch both enabled. Fix it!";
    } else if (!doprocessMcMatch && !doprocessNull) {
      LOG(fatal) << "neither processNull nor processMatch enabled. Fix it!";
    }
  ...
  /// @brief fake process function to be run on data
  void processNull() {}
  PROCESS_SWITCH(HfTrackIndexSkimMcMatchQC, processNull, "fake process match to MC", true);

  using BigTracksMC = soa::Join<aod::BigTracksExtended, aod::BigTracksMC>;
  ...
  PROCESS_SWITCH(HfTrackIndexSkimMcMatchQC, processMcMatch, "process match to MC", false);
};
